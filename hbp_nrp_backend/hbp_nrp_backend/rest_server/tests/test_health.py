...
class TestHealth(RestTest):
    ...
    def testErrorsLast24h(self):
        simulations[0]._Simulation__errors = 1
        simulations[0]._Simulation__creation_datetime = datetime.now(
            tz=timezone('Europe/Zurich')) - timedelta(days=1, seconds=2)
        response = self.client.get('/health/errors-last-24h')
        self.assertEqual(response.status_code, 200)
        self.assertIn("OK", response.data)
        simulations[1]._Simulation__errors = 1
        simulations[1]._Simulation__creation_datetime = datetime.now(
            tz=timezone('Europe/Zurich')) - timedelta(hours=23)
        simulations[2]._Simulation__creation_datetime = datetime.now(
            tz=timezone('Europe/Zurich')) - timedelta(hours=23)
        response = self.client.get('/health/errors-last-24h')
        self.assertEqual(response.status_code, 200)
        self.assertIn("WARNING", response.data)
